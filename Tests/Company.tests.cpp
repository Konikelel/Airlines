...
TEST(company, destructor) {
    ...
    CrewMember crewMember1{pCompany, PILOT, "Victor", "Alb", 1, MALE};
    CrewMember crewMember2{pCompany, STEWARDESS, "Anna", "Vic", 2, FEMALE};
...
TEST(company, plane) {
    ...
    EXPECT_EQ(plane1.getFlights().size(), 0);
    EXPECT_TRUE(existSet(pCompany1->getPlanes(), plane1));
    ...
    EXPECT_EQ(plane1.getFlights().size(), 0);
    EXPECT_FALSE(existSet(pCompany1->getPlanes(), plane1));
    EXPECT_TRUE(existSet(pCompany2->getPlanes(), plane1));
    // removePlane
...
TEST(company, crewMember) {
    ...
    CrewMember crewMember2{PILOT, "Victor", "Alb", 1, MALE};
    ...
    EXPECT_EQ(pCompany1->getPilots().size(), 0);
    EXPECT_TRUE(existSet(pCompany1->getStewardesses(), crewMember1));
    // SET COMPANY, PREVIOUS COMPANY SAME
    ...
    EXPECT_EQ(crewMember1.getFlights().size(), 0);
    EXPECT_FALSE(existSet(pCompany1->getStewardesses(), crewMember1));
    EXPECT_TRUE(existSet(pCompany2->getStewardesses(), crewMember1));
    // removeCrewMember
...
TEST(company, flight) {
    ...
    CrewMember pilot{pCompany, PILOT, "Vic", "Ay", 1, MALE};

    CrewMember stewardess{pCompany, STEWARDESS, "Ki", "Be", 1, FEMALE};

    Passenger passenger{"Ew", "Ao", 1, MALE};
    // createFlights
